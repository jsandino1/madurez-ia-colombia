...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!to || !subject || !html) return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Faltan campos requeridos' }) };

    const r = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${RESEND_KEY}` },
      body: JSON.stringify({
        from: 'MadurezIA Colombia <js@pacte.co>',
        to: [to],
        subject,
        html
      })
    ...
    if (!r.ok) return { statusCode: r.status, headers: CORS, body: JSON.stringify({ error: data }) };
